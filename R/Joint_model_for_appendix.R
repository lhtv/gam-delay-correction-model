...
# Model checking
# QQ plot
mgcv::qq.gam(XZmodel, rep = 100, level = 0.95)  # envelope
r <- residuals(XZmodel, type = "deviance")
r <- r[is.finite(r)]
qqline(r, col = 2, lwd = 2)               # add the red line back
title("Joint model")         # add the header manually

r <- residuals(m, type = "deviance")
r <- r[is.finite(r)]

qqnorm(r, main = "", cex = 0.4, pch = 19)  # smaller points, suppress default title
qqline(r, col = "red")
title("Joint model")   # add your own
...
comp_table <- data.table(
  model = c("Zmodel", "XZmodel"),
  aic = c(AIC(Zmodel), AIC(XZmodel)),
  dev_expl = c(summary(Zmodel)$dev.expl, summary(XZmodel)$dev.expl),
  rmse = c(rmse(truth_Z, fit_Z), rmse(truth_XZ, fit_XZ)),
  mae  = c(mae(truth_Z, fit_Z), mae(truth_XZ, fit_XZ)),
  cov95 = c(cov95(truth_Z, lwr_Z, upr_Z), cov95(truth_XZ, lwr_XZ, upr_XZ)),
  mean_pi_width = c(pi_width(lwr_Z, upr_Z), pi_width(lwr_XZ, upr_XZ))
)
